...
def backlog_metrics(db: Session, start: date, end: date, tester: str | None = None, status: str | None = None, ticket_category: str | None = None) -> list[dict]:
    cohort_start = start.replace(day=1)
    cohort_end = date(end.year, end.month, monthrange(end.year, end.month)[1])
    query = select(TrackerRecord).where(
        TrackerRecord.deleted_at.is_(None),
        TrackerRecord.date_started.is_not(None),
        TrackerRecord.date_started >= cohort_start,
        TrackerRecord.date_started <= cohort_end,
    )
    if tester:
    ...
    def closure_date(record: TrackerRecord) -> date | None:
        if is_active_backlog_status(record.status):
            return None
        return record.date_ended or record.date_started
    ...
    while cursor <= final_month:
        ...
        created_records = [record for record in records if cursor <= record.date_started <= month_end]
        closed = [
            record for record in created_records
            if closure_date(record) and cursor <= closure_date(record) <= month_end
        ]
        backlog = [
            record for record in created_records
            if closure_date(record) is None or closure_date(record) > month_end
        ]
        rows.append({
